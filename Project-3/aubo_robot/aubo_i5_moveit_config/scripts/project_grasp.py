...
def robot_player(color_image, depth_image, ai_move):
    ...
    group.execute(plan)
    pick.pose.position.z = pick.pose.position.z - 0.058
...
if checkerboard_found:
    ...
    vis = cv2.drawChessboardCorners(gray, (4,4), corners_refined, checkerboard_found)
    # Get observed checkerboard center 3D point in camera space
    checkerboard_pix = np.round(corners_refined).astype(int).reshape(16,2)
    # checkerboard_z = np.mean(np.mean(depth_image[checkerboard_pix[8,1]-20:checkerboard_pix[8,1]+20,checkerboard_pix[8,0]-20:checkerboard_pix[8,0]+20])) * depth_scale
    checkerboard_z = 0.996
    ...
    cv2.imshow("Detected chess board!", gray)
    cv2.waitKey(2000)
    cv2.destroyAllWindows()

# cv2.imshow('color_image_background',color_image_background)
# cv2.waitKey(6000)
# cv2.destroyAllWindows()

checkerboard_pix = np.delete(checkerboard_pix,[3, 7, 11, 12,13,14,15],axis=0)
checkerboard_x = np.delete(checkerboard_x,[3, 7, 11, 12,13,14,15])
checkerboard_y = np.delete(checkerboard_y,[3, 7, 11, 12,13,14,15])
...
def detect_person_move(color_image, game):
    for i in range(9):
        ...
        if sum(np.abs(color_image_background[v,u].astype(float) - color_image[v,u].astype(float))>50)>1:
            if game.board[i]==" ":
                person_move = i
